---
title: "Venlafaxine - taxa bar plots"
author: "Milena Esser"
date: "2025-05-20"
output:
  html_document: 
...
load("R_analysis/18S/R_output/ps_objects/Venla_water_phyloseq_filt_unnorm.RData")
load("R_analysis/18S/R_output/ps_objects/Venla_sediment_phyloseq_filt_unnorm.RData")

# Create output directory
output_dir <- "R_analysis/18S/R_output/taxabarplot"
...
```{r plot-phy-biofilm, results='asis', fig.height=6, fig.width=12}
...
df$treatment <- factor(df$treatment, levels=c("control", "4", "26", "138", "3,727", "100,000"))
...
```{r plot-phy-sed, results='asis', fig.height=6, fig.width=12}
#### STEP 1: Aggregate and transform to relative abundance ####
...
```{r plot-biofilm-fam, results='asis', fig.height=6, fig.width=12}
#### STEP 1: Aggregate and transform to relative abundance ####
ps_tax <- tax_glom(ps_biofilm, taxrank = taxrank)
ps_relabund <- transform_sample_counts(ps_tax, function(x) x / sum(x))
df <- psmelt(ps_relabund)
...
```{r plot-sediment-fam, results='asis', fig.height=6, fig.width=12}
#### STEP 1: Aggregate and transform to relative abundance ####
ps_tax <- tax_glom(ps_sediment, taxrank = taxrank)
ps_relabund <- transform_sample_counts(ps_tax, function(x) x / sum(x))
df <- psmelt(ps_relabund)
